...
| Zone | Guarded | Unguarded | Resources | Total |
|---|---:|---:|---:|---:|
...
`Guarded` is the value budget for encounters with external map guards. `Unguarded` is the budget for objects placed without an external guard. `Resources` is reserved for loose resource pickups. `Total` is their sum.
